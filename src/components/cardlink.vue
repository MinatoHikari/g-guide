<template id="link">
  <div :class="{'float-link': linkUP}" class="panel">
    <div  @mouseleave="floatDown" @mouseenter="floatLink" :class="{'float-link-shadow': linkUP}" class="link">
      ...
      <slot name="pic"></slot>
      <div class="linkicon"><i @touchend="copyPropLink" :copylink="link" @touchstart.prevent="showLinkInfo" @mouseenter="showLinkInfo" class="czs-circle"></i></div>
...
export default {
  ...
  methods: {
    ...
    copyPropLink: function(e) {
      ...
      this.$copyText(e.target.getAttribute('copylink')).then(iscopy);
    }
  }
};
</script>
